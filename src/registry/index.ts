...
export class ServerRegistry {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          ...
          logger.warn(
            `Connection attempt ${attempt} failed for ${serverId}, retrying in ${delay}ms:`,
  ...
  private async createHttpConnectionWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          ...
          logger.warn(
            `HTTP connection attempt ${attempt} failed for ${serverId}, retrying in ${delay}ms:`,
            { error: lastError.message }
          );
          await new Promise((resolve) => setTimeout(resolve, delay));
        }
    ...
    throw new ConnectionError(
      serverId,
      `HTTP connection failed after ${maxRetries} attempts: ${lastError?.message}`
    );
  }
  ...
  private async createHttpConnection(
    ...
    try {
      ...
      try {
        ...
        logger.warn('HTTP connection test failed, but connection established:', {
          serverId,
          testError,
        });
        // Continue anyway as some servers may not support immediate testing
  ...
  async callTool(serverId: string, toolName: string, args: Record<string, unknown>): Promise<any> {
    ...
    throw new ConnectionError(
      serverId,
      `Tool call failed: ${lastError instanceof Error ? lastError.message : 'Unknown error'}`
    );
  }
  ...
  private resolveToolCallAttempts(
  ...
  private validateToolArguments(
    ...
    if (missingFields.length > 0) {
      throw new Error(
        `Missing required arguments for ${serverId}/${toolName}: ${missingFields.join(', ')}`
      );
    }
  }
  ...
  private findToolMetadata(serverId: string, toolName: string): ToolMetadata | undefined {
    ...
    return this.getAllTools().find(
      (tool) => tool.server_id === serverId && tool.tool_name === toolName
    );
  }
  ...
  filterTools(query?: string, tags?: string[], serverId?: string): ToolMetadata[] {
    ...
    if (tags && tags.length > 0) {
      tools = tools.filter((tool) => tool.tags?.some((tag) => tags.includes(tag)));
    }
  ...
  getAllServerStatuses(): Record<
    string,
    { connected: boolean; toolCount: number; lastUpdate?: Date }
  > {
