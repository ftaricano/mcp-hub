...
class MockMCPHub {
  ...
  private setupMockServers() {
    ...
    this.serverManager.mockServer('outlook-fernando', {
      result: { content: [{ type: 'text', text: JSON.stringify({ success: true, type: 'email' }) }] },
      latency: 150
    });
    
    this.serverManager.mockServer('trello', {
      result: { content: [{ type: 'text', text: JSON.stringify({ success: true, type: 'card' }) }] },
      latency: 120
    });
    
    this.serverManager.mockServer('spotify', {
      result: { content: [{ type: 'text', text: JSON.stringify({ success: true, type: 'music' }) }] },
      latency: 200
    });
    
    this.serverManager.mockServer('notion', {
      result: { content: [{ type: 'text', text: JSON.stringify({ success: true, type: 'page' }) }] },
      latency: 180
    });
  ...
  async getRecommendations(params: { category?: string; popular?: boolean; recent?: boolean }) {
    ...
    if (params.popular) {
      response.popular_tools = [
        { tool_key: 'outlook-fernando/send_email', usage_count: 150, success_rate: 0.98 },
...
describe('End-to-End Workflow Tests', () => {
  ...
  describe('Workflow: Gestão de Email e Projetos', () => {
    it('deve executar workflow completo: Email → Análise → Trello → Documentação', async () => {
      ...
      expect(emailSearchResult.results[0].server_id).toBe('outlook-fernando');
      ...
      hub.serverManager.mockServer('outlook-fernando', {
        result: {
          content: [{
            type: 'text',
            text: JSON.stringify({
      ...
      const emailResult = await hub.callTool('outlook-fernando', 'list_emails', {
  ...
  describe('Workflow: Produtividade com Música', () => {
    it('deve executar workflow: Música para Foco → Busca de Documentos → Organização', async () => {
      ...
      const fileSearchResult = await hub.smartSearch('listar arquivos importantes na pasta projeto');
      ...
      const organizationResult = await hub.smartSearch('criar página notion para organizar documentos');
  ...
  describe('Workflow: Comunicação e Follow-up', () => {
    it('deve executar workflow: Receber Email → Responder → Agendar Follow-up → WhatsApp', async () => {
      ...
      expect(inboxSearchResult.results[0].server_id).toBe('outlook-fernando');
      ...
      const inboxResult = await hub.callTool('outlook-fernando', 'list_emails', {
      ...
      expect(replySearchResult.results[0].server_id).toBe('outlook-fernando');
      ...
      const replyResult = await hub.callTool('outlook-fernando', 'reply_to_email', {
  ...
  describe('Casos de Uso Reais Complexos', () => {
    it('deve processar cenário real: Reunião de Status → Relatório → Distribuição', async () => {
      ...
      const meetingEmails = await hub.callTool('outlook-fernando', 'list_emails', {
      ...
      const statusDoc = await hub.callTool('notion', 'create_page', {
        ...
        content: {
          ...
          participants: ['fernando@empresa.com', 'gerente@empresa.com'],
      ...
      const summaryEmail = await hub.callTool('outlook-fernando', 'send_email', {
    ...
    it('deve gerenciar workflow de crise: Problema → Notificação → Documentação → Follow-up', async () => {
      ...
      expect(criticalEmail.results[0].server_id).toBe('outlook-fernando');
