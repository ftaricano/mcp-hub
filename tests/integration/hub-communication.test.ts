...
describe('Hub Server Communication Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockServerManager.mockServer('email-primary', {
      tools: MOCK_TOOLS_BY_SERVER['email-primary'],
    ...
    vi.spyOn(serverRegistry, 'getConnection').mockImplementation(async (serverId: string) => ({
      client: {
        callTool: async ({
          name,
          arguments: args,
    ...
    vi.spyOn(serverRegistry, 'getAllTools').mockReturnValue([
      ...
      ...MOCK_TOOLS_BY_SERVER['email-primary'].map((tool) => ({
        server_id: 'email-primary',
        server_name: 'Primary Email',
        tool_name: tool.name,
        description: tool.description,
        schema: tool.inputSchema,
        tags: ['email'],
  ...
  describe('Server Health and Connectivity', () => {
    it('deve estabelecer conexão com todos os servidores mockados', async () => {
      const servers = ['trello', 'email-primary', 'spotify'];
    ...
    it('deve medir latência de cada servidor corretamente', async () => {
      const servers = [
        ...
        { id: 'email-primary', expectedLatency: 150 },
  ...
  describe('Cross-Server Tool Execution', () => {
    it('deve executar ferramentas em diferentes servidores sequencialmente', async () => {
      // 1. Buscar emails
      const emailResult = await serverRegistry.callTool('email-primary', 'list_emails', {
        search: 'projeto',
        limit: 5,
      });
    ...
    it('deve executar operações paralelas sem conflitos', async () => {
      const operations = [
        ...
        serverRegistry.callTool('email-primary', 'list_emails', { limit: 10 }),
  ...
  describe('Cache Integration Across Servers', () => {
    it('deve cachear resultados por servidor independentemente', async () => {
      ...
      const emailResult1 = await serverRegistry.callTool('email-primary', 'list_emails', emailArgs);
      ...
      hubCache.setToolResult('email-primary', 'list_emails', emailHash, emailResult1);

      // Second calls - should hit cache
      const trelloResult2 = hubCache.getToolResult('trello', 'get_boards', trelloHash);
      const emailResult2 = hubCache.getToolResult('email-primary', 'list_emails', emailHash);
  ...
  describe('Error Handling and Recovery', () => {
    it('deve propagar erros específicos de cada servidor', async () => {
      ...
      await expect(serverRegistry.callTool('trello', 'get_boards', {})).rejects.toThrow(
    ...
    it('não faz retry automático para tool calls mutáveis por padrão', async () => {
      ...
      await expect(
        serverRegistry.callTool('trello', 'create_card', { title: 'urgent' })
  ...
  describe('Performance Under Load', () => {
    it('deve manter performance com múltiplas chamadas simultâneas', async () => {
      ...
      for (let i = 0; i < concurrentCalls; i++) {
        const serverId = ['trello', 'email-primary', 'spotify'][i % 3];
        const toolName =
          serverId === 'trello'
            ? 'get_boards'
            : serverId === 'email-primary'
              ? 'list_emails'
              : 'search_tracks';

        operations.push(
          serverRegistry.callTool(
            serverId,
            toolName,
  ...
  describe('Data Consistency and Validation', () => {
    ...
    it('deve manter integridade de dados em workflows cross-server', async () => {
      ...
      const emailResult = await serverRegistry.callTool('email-primary', 'send_email', {
        to: 'test@example.com',
